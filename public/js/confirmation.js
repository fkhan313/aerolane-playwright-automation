document.addEventListener('DOMContentLoaded', () => {
  ...
  codeEl.innerHTML = code.split('').map(ch => `<span class="cell mono">${ch}</span>`).join('');
  ...
  document.getElementById('itinerary-summary').innerHTML =
    legSummary(booking.outbound, 'Outbound') + (booking.returnFlight ? legSummary(booking.returnFlight, 'Return') : '');

  document.getElementById('passenger-summary').innerHTML = booking.passengers.map(p => `
